...
class Library(QtWidgets.QMainWindow):
    ...
    def loadLibrary(self):
        print("[DEBUG] loadLibrary called")
        print("[DEBUG] librarydir =", self.useData.appPathDict.get("librarydir"))
        self.snippetsListWidget.clear()
        library_path = self.useData.appPathDict.get("librarydir")
        if not os.path.isdir(library_path):
            print("[WARN] librarydir does not exist:", library_path)
            return
        files = sorted(os.listdir(library_path))
        print("[DEBUG] files found in librarydir:", files)
        alpha = {}
        for i in files:
            c = list(i)[0].upper()
            if c not in alpha:
